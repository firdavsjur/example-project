...
func (s *shopCartRepo) AddShopCart(req *models.Add) (string, error) {
	...
	err = os.WriteFile(s.fileName, body, os.ModePerm)
	if err != nil {
		return "", err
	}
	return uuid, nil
...
// GetAllShopCart
func (s *shopCartRepo) GetAllShopCart(req *models.GetAllShopCartRequest) (models.GetAllShopCartsResponse, error) {
	shopCarts, err := s.Read()
	if err != nil {
		return models.GetAllShopCartsResponse{}, err
	}

	return models.GetAllShopCartsResponse{
		Count:     len(shopCarts),
		ShopCarts: shopCarts,
	}, nil
}
